...
fun main(args: Array<String>): Unit = runBlocking {
    ...
    val select = launch {
        ...
        // Mas sofisticado
        repository.findAll(1, 100).filter { it.first_name.first() == 'G' }
            .take(2)
            .collect {
                println(it)
            }
    ...
    println("Update user")
    res = res.copy(first_name = "Janet", email = "updted@updted.com")
    ...
    // Nos autenticamos
    println("Login")
    val login = async { KtorFitClient.login(LoginDto("eve.holt@reqres.in", "cityslicka")) }
