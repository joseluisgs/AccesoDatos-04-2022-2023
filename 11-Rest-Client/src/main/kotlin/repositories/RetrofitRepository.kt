...
import services.retrofit.RetroFitClient
...
class RetrofitRepository : CrudRepository<User, Long> {
    ...
    override suspend fun findAll(page: Int, perPage: Int): Flow<User> = withContext(Dispatchers.IO) {
        ...
        if (call.isSuccessful && res?.data != null) {
            ...
            logger.error { "findAll(page=$page, perPage=$perPage) - ${call.code()} - ${call.isSuccessful}" }
    ...
    suspend fun findAllWithToken(token: String, page: Int, perPage: Int): Flow<User> {
        ...
        if (call.isSuccessful && res?.data != null) {
            ...
            logger.error { "findAllWithToken(token=$token, page=$page, perPage=$perPage) - ${call.code()} - ${call.isSuccessful}" }
            throw RestException("Error al obtener los usuarios: ${call.errorBody()}")
        }
    }
    ...
    override suspend fun findById(id: Long): User {
        ...
        if (call.isSuccessful && res?.data != null) {
            ...
            logger.error { "findById(id=$id) - ${call.code()} - ${call.isSuccessful}" }
    ...
    override suspend fun save(entity: User): User {
        ...
        if (call.isSuccessful && res != null) {
            ...
            logger.error { "save(entity=$entity) - ${call.code()} - ${call.isSuccessful}" }
    ...
    override suspend fun update(entity: User): User {
        ...
        if (call.isSuccessful && res != null) {
            ...
            logger.error { "update(entity=$entity) - ${call.code()} - ${call.isSuccessful}" }
    ...
    override suspend fun delete(entity: User): User {
        ...
        val call = client.delete(entity.id)
        if (call.isSuccessful) {
            logger.debug { "delete(entity=$entity) - ${call.code()} - ${call.isSuccessful}" }
            return entity
        } else {
            logger.error { "delete(entity=$entity) - ${call.code()} - ${call.isSuccessful}" }
